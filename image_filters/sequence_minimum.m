function I_Out=sequence_minimum(image_directory,image_format,frame_min,frame_max)
% This function calculates the minimum of a sequence of images in the
...
% be neglected to calculate the minimum of the entire image sequence.
% Alternatively to load the entire image to the end, frame_max may be set
% to Inf.  The minimum is returned in the same format as the image format, ie 
% uint8 or uint16.
%
...
for n=1:win_num
    ...
    % Compute the minimum of each pixel in time
    I_Min = min(I_Win,[],3);

    % This saves the minimum window to the output image
    I_Out(ii_min:ii_max,jj_min:jj_max)=I_Min;
end