function [p]=pressure_lines_4_3d(x,y,z,u,v,w,Re,dt,solver,bclist,Pref,Px,Py)
%function [p]=pressure_solve(x,y,u,v,Re,dt,[solver],[bclist],[Pref, Px, Py])
%
% Returns p(x,y,t,z) from u(x,y,t,z) and v(x,y,t,z) using a multi-directional 
...
% changed program to allow 3d velocity fields in multiple planes, each
%  plane is still computed independently (pressures not linked in z)
...
M = NI;      %number of points in x
N = NJ;      %number of points in y
L = NK;
T = NT;      %number of points in t
...
% dwdt(:,:,1    ,:) = ( -w(:,:,3  ,:) +4*w(:,:, 2 ,:) -3*w(:,:,1    ,:)) / (2*dt);
% dwdt(:,:,2:T-1,:) = (  w(:,:,3:T,:)                 -  w(:,:,1:T-2,:)) / (2*dt);
% dwdt(:,:,    T,:) = (3*w(:,:,  T,:) -4*w(:,:,T-1,:) +  w(:,:,  T-2,:)) / (2*dt);
...
% dwdx(1    ,:,:,:) = ( -w(3  ,:,:,:) +4*w( 2 ,:,:,:) -3*w(1    ,:,:,:)) / (2*dx);
% dwdx(2:M-1,:,:,:) = (  w(3:M,:,:,:)                 -  w(1:M-2,:,:,:)) / (2*dx);
% dwdx(M    ,:,:,:) = (3*w(M  ,:,:,:) -4*w(M-1,:,:,:) +  w(  M-2,:,:,:)) / (2*dx);
...
% dwdy(:,1    ,:,:) = ( -w(:,3  ,:,:) +4*w(:, 2 ,:,:) -3*w(:,1    ,:,:)) / (2*dy);
% dwdy(:,2:N-1,:,:) = (  w(:,3:N,:,:)                 -  w(:,1:N-2,:,:)) / (2*dy);
% dwdy(:,    N,:,:) = (3*w(:,  N,:,:) -4*w(:,N-1,:,:) +  w(:,  N-2,:,:)) / (2*dy);

dudz(:,:,:,1    ) = ( -u(:,:,:,3  ) +4*u(:,:,:, 2 ) -3*u(:,:,:,1    )) / (2*dz);
dudz(:,:,:,2:L-1) = (  u(:,:,:,3:L)                 -  u(:,:,:,1:L-2)) / (2*dz);
dudz(:,:,:,    L) = (3*u(:,:,:,  L) -4*u(:,:,:,L-1) +  u(:,:,:,  L-2)) / (2*dz);

dvdz(:,:,:,1    ) = ( -v(:,:,:,3  ) +4*v(:,:,:, 2 ) -3*v(:,:,:,1    )) / (2*dz);
dvdz(:,:,:,2:L-1) = (  v(:,:,:,3:L)                 -  v(:,:,:,1:L-2)) / (2*dz);
dvdz(:,:,:,    L) = (3*v(:,:,:,  L) -4*v(:,:,:,L-1) +  v(:,:,:,  L-2)) / (2*dz);

dwdz(:,:,:,1    ) = ( -w(:,:,:,3  ) +4*w(:,:,:, 2 ) -3*w(:,:,:,1    )) / (2*dz);
dwdz(:,:,:,2:L-1) = (  w(:,:,:,3:L)                 -  w(:,:,:,1:L-2)) / (2*dz);
dwdz(:,:,:,    L) = (3*w(:,:,:,  L) -4*w(:,:,:,L-1) +  w(:,:,:,  L-2)) / (2*dz);
...
% d2udz2(:,:,:,1    ) = (u(:,:,:,3  ) -2*u(:,:,:,2    ) + u(:,:,:,1    )) / (dz^2);
% d2udz2(:,:,:,2:L-1) = (u(:,:,:,3:L) -2*u(:,:,:,2:L-1) + u(:,:,:,1:L-2)) / (dz^2);
% d2udz2(:,:,:,    L) = (u(:,:,:,  L) -2*u(:,:,:,  L-1) + u(:,:,:,  L-2)) / (dz^2);
...
% d2vdz2(:,:,:,1    ) = (v(:,:,:,3  ) -2*v(:,:,:,2    ) + v(:,:,:,1    )) / (dz^2);
% d2vdz2(:,:,:,2:L-1) = (v(:,:,:,3:L) -2*v(:,:,:,2:L-1) + v(:,:,:,1:L-2)) / (dz^2);
% d2vdz2(:,:,:,    L) = (v(:,:,:,  L) -2*v(:,:,:,  L-1) + v(:,:,:,  L-2)) / (dz^2);

% d2wdx2(1    ,:,:,:) = (w(3  ,:,:,:) -2*w(2    ,:,:,:) + w(1    ,:,:,:)) / (dx^2);
% d2wdx2(2:M-1,:,:,:) = (w(3:M,:,:,:) -2*w(2:M-1,:,:,:) + w(1:M-2,:,:,:)) / (dx^2);
% d2wdx2(M    ,:,:,:) = (w(M  ,:,:,:) -2*w(  M-1,:,:,:) + w(  M-2,:,:,:)) / (dx^2);

% d2wdy2(:,1    ,:,:) = (w(:,3  ,:,:) -2*w(:,2    ,:,:) + w(:,1    ,:,:)) / (dy^2);
% d2wdy2(:,2:N-1,:,:) = (w(:,3:N,:,:) -2*w(:,2:N-1,:,:) + w(:,1:N-2,:,:)) / (dy^2);
% d2wdy2(:,    N,:,:) = (w(:,  N,:,:) -2*w(:,  N-1,:,:) + w(:,  N-2,:,:)) / (dy^2);

d2wdz2(:,:,:,1    ) = (w(:,:,:,3  ) -2*w(:,:,:,2    ) + w(:,:,:,1    )) / (dz^2);
d2wdz2(:,:,:,2:L-1) = (w(:,:,:,3:L) -2*w(:,:,:,2:L-1) + w(:,:,:,1:L-2)) / (dz^2);
d2wdz2(:,:,:,    L) = (w(:,:,:,  L) -2*w(:,:,:,  L-1) + w(:,:,:,  L-2)) / (dz^2);
...
% duwdx(1    ,:,:,:) = ( -uw(3  ,:,:,:) +4*uw( 2 ,:,:,:) -3*uw(1    ,:,:,:)) / (2*dx);
% duwdx(2:M-1,:,:,:) = (  uw(3:M,:,:,:)                  -  uw(1:M-2,:,:,:)) / (2*dx);
% duwdx(M    ,:,:,:) = (3*uw(M  ,:,:,:) -4*uw(M-1,:,:,:) +  uw(  M-2,:,:,:)) / (2*dx);
...
% dvwdy(:,1    ,:,:) = ( -vw(:,3  ,:,:) +4*vw(:, 2 ,:,:) -3*vw(:,1    ,:,:)) / (2*dy);
% dvwdy(:,2:N-1,:,:) = (  vw(:,3:N,:,:)                  -  vw(:,1:N-2,:,:)) / (2*dy);
% dvwdy(:,    N,:,:) = (3*vw(:,  N,:,:) -4*vw(:,N-1,:,:) +  vw(:,  N-2,:,:)) / (2*dy);

duwdz(:,:,:,1    ) = ( -uw(:,:,:,3  ) +4*uw(:,:,:, 2 ) -3*uw(:,:,:,1    )) / (2*dz);
duwdz(:,:,:,2:L-1) = (  uw(:,:,:,3:L)                  -  uw(:,:,:,1:L-2)) / (2*dz);
duwdz(:,:,:,    L) = (3*uw(:,:,:,  L) -4*uw(:,:,:,L-1) +  uw(:,:,:,  L-2)) / (2*dz);

dvwdz(:,:,:,1    ) = ( -vw(:,:,:,3  ) +4*vw(:,:,:, 2 ) -3*vw(:,:,:,1    )) / (2*dz);
dvwdz(:,:,:,2:L-1) = (  vw(:,:,:,3:L)                  -  vw(:,:,:,1:L-2)) / (2*dz);
dvwdz(:,:,:,    L) = (3*vw(:,:,:,  L) -4*vw(:,:,:,L-1) +  vw(:,:,:,  L-2)) / (2*dz);

% dwwdz(:,:,:,1    ) = ( -ww(:,:,:,3  ) +4*ww(:,:,:, 2 ) -3*ww(:,:,:,1    )) / (2*dz);
% dwwdz(:,:,:,2:L-1) = (  ww(:,:,:,3:L)                  -  ww(:,:,:,1:L-2)) / (2*dz);
% dwwdz(:,:,:,    L) = (3*ww(:,:,:,  L) -4*ww(:,:,:,L-1) +  ww(:,:,:,  L-2)) / (2*dz);
...
%Unused addition
% d2vdydx(1    ,:,:) = ( -dvdy(3  ,:,:) +4*dvdy( 2 ,:,:) -3*dvdy(1    ,:,:)) / (2*dx);
% d2vdydx(2:M-1,:,:) = (  dvdy(3:M,:,:)                  -  dvdy(1:M-2,:,:)) / (2*dx);
% d2vdydx(M    ,:,:) = (3*dvdy(M  ,:,:) -4*dvdy(M-1,:,:) +  dvdy(  M-2,:,:)) / (2*dx);
% 
% d2udxdy(1    ,:,:) = ( -dudx(3  ,:,:) +4*dudx( 2 ,:,:) -3*dudx(1    ,:,:)) / (2*dy);
% d2udxdy(2:M-1,:,:) = (  dudx(3:M,:,:)                  -  dudx(1:M-2,:,:)) / (2*dy);
% d2udxdy(M    ,:,:) = (3*dudx(M  ,:,:) -4*dudx(M-1,:,:) +  dudx(  M-2,:,:)) / (2*dy);
...
for n=1:length(bclist)
    ...
    %first point in list
        ...
        if nhat(2) < -flat      % north
            ...
            %dvwdy  (xi, yi,:,:)    = (3*vw(xi, yi,:,:) -4*vw(xi, yi-1,:,:) + vw(xi, yi-2,:,:)) / (2*dy);
            %d2vvdy2(xi, yi,:,:)    = (  vv(xi, yi,:,:) -2*vv(xi, yi-1,:,:) + vv(xi, yi-2,:,:)) / (dy^2);
            %d2vwdy2(xi, yi,:,:)    = (  vw(xi, yi,:,:) -2*vw(xi, yi-1,:,:) + vw(xi, yi-2,:,:)) / (dy^2);
        ...
        elseif nhat(2) > flat   % south
            ...
            %dvwdy  (xi, yi,:,:) = (-vw(xi, yi+2,:,:) +4*vw(xi, yi+1,:,:) -3*vw(xi, yi,:,:)) / (2*dy);
            %d2vvdy2(xi, yi,:,:) = ( vv(xi, yi+2,:,:) -2*vv(xi, yi+1,:,:) +  vv(xi, yi,:,:)) / (dy^2);
            %d2vwdy2(xi, yi,:,:) = ( vw(xi, yi+2,:,:) -2*vw(xi, yi+1,:,:) +  vw(xi, yi,:,:)) / (dy^2);
        ...
        if nhat(1) > flat       %west
            ...
            %duwdx  (xi ,yi,:,:) = (-uw(xi+2, yi,:,:) +4*uw(xi+1, yi,:,:) -3*uw(xi, yi,:,:)) / (2*dx);
            %d2uudx2(xi ,yi,:,:) = ( uu(xi+2, yi,:,:) -2*uu(xi+1, yi,:,:) +  uu(xi, yi,:,:)) / (dx^2);
            %d2uwdx2(xi ,yi,:,:) = ( uw(xi+2, yi,:,:) -2*uw(xi+1, yi,:,:) +  uw(xi, yi,:,:)) / (dx^2);
        ...
        elseif nhat(1) < -flat  %east
            ...
            %duwdx  (xi, yi,:,:) = (3*uw(xi, yi,:,:) -4*uw(xi-1, yi,:,:) + uw(xi-2, yi,:,:)) / (2*dx);
            %d2uudx2(xi, yi,:) = (  uu(xi, yi,:) -2*uu(xi-1, yi,:) + uu(xi-2, yi,:)) / (dx^2);
            %d2uwdx2(xi, yi,:) = (  uw(xi, yi,:) -2*uw(xi-1, yi,:) + uw(xi-2, yi,:)) / (dx^2);
...
%cross derivatives only useful for conservative2
...
if strcmp(solver,'conservative')
    f = 1/Re * (d2udx2+d2udy2+d2wdz2) - (dudt + duudx + duvdy + duwdz);
    g = 1/Re * (d2vdx2+d2vdy2+d2wdz2) - (dvdt + duvdx + dvvdy + dvwdz);
elseif strcmp(solver,'mixed')
    f = 1/Re * (d2udx2+d2udy2) - ( dudt + 1/2*(u.*dudx + v.*dudy + w.*dudz) + 1/2*(duudx + duvdy + duwdz) );
    g = 1/Re * (d2vdx2+d2vdy2) - ( dvdt + 1/2*(u.*dvdx + v.*dvdy + w.*dvdz) + 1/2*(duvdx + dvvdy + dvwdz) );
elseif strcmp(solver,'standard')
    f = 1/Re * (d2udx2+d2udy2) - ( dudt + u.*dudx + v.*dudy + w.*dudz );
    g = 1/Re * (d2vdx2+d2vdy2) - ( dvdt + u.*dvdx + v.*dvdy + w.*dvdz );
else
    error('unknown flowtype');
end
...
for t=1:NT
    for k=1:NK
        ...
        j=1;
        for i=2:NI
                p1(i,j) =    p1(i-1,j  ) + 1/2*(f(i-1,j  ,t,k)+f(i,j,t,k))*dx;                                          %from W
        end
        for j=2:NJ
            i = 1;
                p1(i,j) = (  p1(i  ,j-1)                                      + 1/2*(g(i  ,j-1,t,k)+g(i,j,t,k))*dy  ... %from S
                           + p1(i+1,j-1) - 1/2*(f(i+1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i+1,j-1,t,k)+g(i,j,t,k))*dy )/2; %from SE
            for i=2:NI-1
                p1(i,j) = (  p1(i-1,j  ) + 1/2*(f(i-1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from W
                           + p1(i-1,j-1) + 1/2*(f(i-1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i-1,j-1,t,k)+g(i,j,t,k))*dy ...  %from SW
                           + p1(i  ,j-1)                                      + 1/2*(g(i  ,j-1,t,k)+g(i,j,t,k))*dy ...; %from S
                           + p1(i+1,j-1) - 1/2*(f(i+1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i+1,j-1,t,k)+g(i,j,t,k))*dy )/4; %from SE
            end
            i = NI;
                p1(i,j) = (  p1(i-1,j  ) + 1/2*(f(i-1,j  ,t,k)+f(i,j,t,k))*dx                                       ... %from W
                           + p1(i-1,j-1) + 1/2*(f(i-1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i-1,j-1,t,k)+g(i,j,t,k))*dy  ... %from SW
                           + p1(i  ,j-1)                                      + 1/2*(g(i  ,j-1,t,k)+g(i,j,t,k))*dy )/3; %from S
        end
        ...
        i=1;
        for j=2:NJ
                p5(i,j) =    p5(i  ,j-1)                                      + 1/2*(g(i  ,j-1,t,k)+g(i,j,t,k))*dy;     %from S
        end
        for i=2:NI
            j = 1;
                p5(i,j) = (  p5(i-1,j  ) + 1/2*(f(i-1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from W
                           + p5(i-1,j+1) + 1/2*(f(i-1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i-1,j+1,t,k)+g(i,j,t,k))*dy )/2; %from NW
            for j=2:NJ-1
                p5(i,j) = (  p5(i-1,j  ) + 1/2*(f(i-1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from W
                           + p5(i-1,j-1) + 1/2*(f(i-1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i-1,j-1,t,k)+g(i,j,t,k))*dy ...  %from SW
                           + p5(i  ,j-1)                                      + 1/2*(g(i  ,j-1,t,k)+g(i,j,t,k))*dy ...; %from S
                           + p5(i-1,j+1) + 1/2*(f(i-1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i-1,j+1,t,k)+g(i,j,t,k))*dy )/4; %from NW
            end
            j = NJ;
                p5(i,j) = (  p5(i-1,j  ) + 1/2*(f(i-1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from W
                           + p5(i-1,j-1) + 1/2*(f(i-1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i-1,j-1,t,k)+g(i,j,t,k))*dy ...  %from SW
                           + p5(i  ,j-1)                                      + 1/2*(g(i  ,j-1,t,k)+g(i,j,t,k))*dy )/3; %from S
        end

        %from SE->SW corner
        j=1;
        for i=NI-1:-1:1
                p2(i,j) =    p2(i+1,j  ) - 1/2*(f(i+1,j  ,t,k)+f(i,j,t,k))*dx;                                          %from E
        end
        for j=2:NJ
            i = NI;
                p2(i,j) = (  p2(i  ,j-1)                                      + 1/2*(g(i  ,j-1,t,k)+g(i,j,t,k))*dy ...  %from S
                           + p2(i-1,j-1) - 1/2*(f(i-1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i-1,j-1,t,k)+g(i,j,t,k))*dy )/2; %from SW
            for i=NI-1:-1:2
                p2(i,j) = (  p2(i+1,j  ) - 1/2*(f(i+1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from E
                           + p2(i+1,j-1) - 1/2*(f(i+1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i+1,j-1,t,k)+g(i,j,t,k))*dy ...  %from SE
                           + p2(i  ,j-1)                                      + 1/2*(g(i  ,j-1,t,k)+g(i,j,t,k))*dy ...; %from S
                           + p2(i-1,j-1) + 1/2*(f(i-1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i-1,j-1,t,k)+g(i,j,t,k))*dy )/4; %from SW
            end
            i = 1;
                p2(i,j) = (  p2(i+1,j  ) - 1/2*(f(i+1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from E
                           + p2(i+1,j-1) - 1/2*(f(i+1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i+1,j-1,t,k)+g(i,j,t,k))*dy ...  %from SE
                           + p2(i  ,j-1)                                      + 1/2*(g(i  ,j-1,t,k)+g(i,j,t,k))*dy )/3; %from S
        end

        %from SE->NE corner
        i=NI;
        for j=2:1:NJ
                p6(i,j) =    p6(i  ,j-1)                                      + 1/2*(g(i  ,j-1,t,k)+g(i,j,t,k))*dy;     %from S
        end
        for i=NI-1:-1:1
            j = 1;
                p6(i,j) = (  p6(i+1,j  ) - 1/2*(f(i+1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from E
                           + p6(i+1,j+1) - 1/2*(f(i+1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i+1,j+1,t,k)+g(i,j,t,k))*dy )/2; %from NE
            for j=2:1:NJ-1
                p6(i,j) = (  p6(i+1,j  ) - 1/2*(f(i+1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from E
                           + p6(i+1,j+1) - 1/2*(f(i+1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i+1,j+1,t,k)+g(i,j,t,k))*dy ...  %from NE
                           + p6(i  ,j-1)                                      + 1/2*(g(i  ,j-1,t,k)+g(i,j,t,k))*dy ...  %from S
                           + p6(i+1,j-1) - 1/2*(f(i+1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i+1,j-1,t,k)+g(i,j,t,k))*dy )/4; %from SE
            end
            j = NJ;
                p6(i,j) = (  p6(i+1,j  ) - 1/2*(f(i+1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from E
                           + p6(i+1,j-1) - 1/2*(f(i+1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i+1,j-1,t,k)+g(i,j,t,k))*dy ...  %from SE
                           + p6(i  ,j-1)                                      + 1/2*(g(i  ,j-1,t,k)+g(i,j,t,k))*dy )/3; %from S
        end

        %from NE->NW corner
        j=NJ;
        for i=NI-1:-1:1
                p3(i,j) =    p3(i+1,j  ) - 1/2*(f(i+1,j  ,t,k)+f(i,j,t,k))*dx;                                          %from E
        end
        for j=NJ-1:-1:1
            i = NI;
                p3(i,j) = (  p3(i  ,j+1)                                      - 1/2*(g(i  ,j+1,t,k)+g(i,j,t,k))*dy ...  %from N
                           + p3(i-1,j+1) + 1/2*(f(i-1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i-1,j+1,t,k)+g(i,j,t,k))*dy )/2; %from NW
            for i=NI-1:-1:2
                p3(i,j) = (  p3(i+1,j  ) - 1/2*(f(i+1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from E
                           + p3(i+1,j+1) - 1/2*(f(i+1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i+1,j+1,t,k)+g(i,j,t,k))*dy ...  %from NE
                           + p3(i  ,j+1)                                      - 1/2*(g(i  ,j+1,t,k)+g(i,j,t,k))*dy ...  %from N
                           + p3(i-1,j+1) + 1/2*(f(i-1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i-1,j+1,t,k)+g(i,j,t,k))*dy )/4; %from NW
            end
            i = 1;
                p3(i,j) = (  p3(i+1,j  ) - 1/2*(f(i+1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from E
                           + p3(i+1,j+1) - 1/2*(f(i+1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i+1,j+1,t,k)+g(i,j,t,k))*dy ...  %from NE
                           + p3(i  ,j+1)                                      - 1/2*(g(i  ,j+1,t,k)+g(i,j,t,k))*dy )/3; %from N
        end

        %from NE->SE corner
        i=NI;
        for j=NJ-1:-1:1
                p7(i,j) =    p7(i  ,j+1)                                      - 1/2*(g(i  ,j+1,t,k)+g(i,j,t,k))*dy;     %from N
        end
        for i=NI-1:-1:1
            j = NJ;
                p7(i,j) = (  p7(i+1,j  ) - 1/2*(f(i+1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from E
                           + p7(i+1,j-1) - 1/2*(f(i+1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i+1,j-1,t,k)+g(i,j,t,k))*dy )/2; %from SE
            for j=NJ-1:-1:2
                p7(i,j) = (  p7(i+1,j  ) - 1/2*(f(i+1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from E
                           + p7(i+1,j+1) - 1/2*(f(i+1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i+1,j+1,t,k)+g(i,j,t,k))*dy ...  %from NE
                           + p7(i  ,j+1)                                      - 1/2*(g(i  ,j+1,t,k)+g(i,j,t,k))*dy ...  %from N
                           + p7(i+1,j-1) - 1/2*(f(i+1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i+1,j-1,t,k)+g(i,j,t,k))*dy )/4; %from SE
            end
            j = 2;
                p7(i,j) = (  p7(i+1,j  ) - 1/2*(f(i+1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from E
                           + p7(i+1,j+1) - 1/2*(f(i+1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i+1,j+1,t,k)+g(i,j,t,k))*dy ...  %from NE
                           + p7(i  ,j+1)                                      - 1/2*(g(i  ,j+1,t,k)+g(i,j,t,k))*dy )/3; %from N
        end

        %from NW->SW corner
        i=1;
        for j=NJ-1:-1:1
                p4(i,j) =    p4(i  ,j+1)                                      - 1/2*(g(i  ,j+1,t,k)+g(i,j,t,k))*dy;     %from N
        end
        for i=2:1:NI
            j = NJ;
                p4(i,j) = (  p4(i-1,j  ) + 1/2*(f(i-1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from W
                           + p4(i-1,j-1) + 1/2*(f(i-1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i-1,j-1,t,k)+g(i,j,t,k))*dy )/2; %from SW
            for j=NJ-1:-1:2
                p4(i,j) = (  p4(i-1,j  ) + 1/2*(f(i-1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from W
                           + p4(i-1,j-1) + 1/2*(f(i-1,j-1,t,k)+f(i,j,t,k))*dx + 1/2*(g(i-1,j-1,t,k)+g(i,j,t,k))*dy ...  %from SW
                           + p4(i  ,j+1)                                      - 1/2*(g(i  ,j+1,t,k)+g(i,j,t,k))*dy ...  %from N
                           + p4(i-1,j+1) + 1/2*(f(i-1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i-1,j+1,t,k)+g(i,j,t,k))*dy )/4; %from NW
            end
            j = 2;
                p4(i,j) = (  p4(i-1,j  ) + 1/2*(f(i-1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from W
                           + p4(i  ,j+1)                                      - 1/2*(g(i  ,j+1,t,k)+g(i,j,t,k))*dy ...  %from N
                           + p4(i-1,j+1) + 1/2*(f(i-1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i-1,j+1,t,k)+g(i,j,t,k))*dy )/3; %from NW
        end
        %from NW->NE corner
        j=NJ;
        for i=2:1:NI
                p8(i,j) =    p8(i-1,j  ) + 1/2*(f(i-1,j  ,t,k)+f(i,j,t,k))*dx;                                          %from W
        end
        for j=NJ-1:-1:1
            i = 1;
                p8(i,j) = (  p8(i+1,j+1) - 1/2*(f(i+1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i+1,j+1,t,k)+g(i,j,t,k))*dy ...  %from NE
                           + p8(i  ,j+1)                                      - 1/2*(g(i  ,j+1,t,k)+g(i,j,t,k))*dy )/2; %from N
            for i=2:1:NI-1
                p8(i,j) = (  p8(i-1,j  ) + 1/2*(f(i-1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from W
                           + p8(i+1,j+1) - 1/2*(f(i+1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i+1,j+1,t,k)+g(i,j,t,k))*dy ...  %from NE
                           + p8(i  ,j+1)                                      - 1/2*(g(i  ,j+1,t,k)+g(i,j,t,k))*dy ...  %from N
                           + p8(i-1,j+1) + 1/2*(f(i-1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i-1,j+1,t,k)+g(i,j,t,k))*dy )/4; %from NW
            end
            i = NI;
                p8(i,j) = (  p8(i-1,j  ) + 1/2*(f(i-1,j  ,t,k)+f(i,j,t,k))*dx                                      ...  %from W
                           + p8(i  ,j+1)                                      - 1/2*(g(i  ,j+1,t,k)+g(i,j,t,k))*dy ...  %from N
                           + p8(i-1,j+1) + 1/2*(f(i-1,j+1,t,k)+f(i,j,t,k))*dx - 1/2*(g(i-1,j+1,t,k)+g(i,j,t,k))*dy )/3; %from NW
        end
        ...
        p(:,:,t,k) = (p1+p2+p3+p4+p5+p6+p7+p8)/8;

        %use an offset to peg the reference pressure over time...
        %is this okay to do?
        if nargin>=9
            %pause
            deltaP = Pref(t) - p(Px,Py,t,k);
            p(:,:,t,k) = p(:,:,t,k) + deltaP * ones(NI,NJ);
        end

        %convergence information
        fprintf('  %4i  %4i  %11.4g  %4i  %10.3f\n',t,k,relres,flag,toc)
