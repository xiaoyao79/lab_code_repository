...
%
% Returns p(x,y,t) from u(x,y,t) and v(x,y,t) using GMRES method
...
d2vdxdy(2:M-1,:,:) = (  dvdx(3:M,:,:)                  -  dvdx(1:M-2,:,:)) / (2*dy);
d2vdxdy(M    ,:,:) = (3*dvdx(M  ,:,:) -4*dvdx(M-1,:,:) +  dvdx(  M-2,:,:)) / (2*dy);
...
for t=1:NT
    ...
    [phi,flag,relres,iter,resvec] = gmres(AA, reshape(S(:,:,t),NI*NJ,1), restarts, tol, max_iterations, [], [], phi);
